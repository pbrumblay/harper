...
async function install() {
	console.error(HDB_PROMPT_MSG(LINE_BREAK + INSTALL_START_MSG + LINE_BREAK));
	...
	console.error(HDB_PROMPT_MSG(LINE_BREAK + INSTALL_COMPLETE_MSG + LINE_BREAK));
...
function displayCmdEnvVar(value, msg) {
	if (value !== undefined) {
		if (msg.includes('password')) {
			console.error(`${HDB_PROMPT_MSG(msg)} ${chalk.gray('[hidden]')}`);
			hdbLogger.trace(`${HDB_PROMPT_MSG(msg)} [hidden]`);
		} else {
			console.error(`${HDB_PROMPT_MSG(msg)} ${value}`);
...
async function checkForExistingInstall() {
	...
	if (hdbExists && !ignoreExisting) {
		...
		if (upgradeObj) {
			...
			console.error(LINE_BREAK + chalk.magenta.bold(UPGRADE_MSG));
			console.error(chalk.magenta.bold(upgradeToVerMsg));
			hdbLogger.error(upgradeToVerMsg);
		} else {
			console.error(LINE_BREAK + chalk.magenta.bold(HDB_EXISTS_MSG));
